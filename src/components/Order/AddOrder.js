...
function AddOrder(props) {
        ...
        let d = {
          ...
          description: (
            <>
              ...
              >
                <FiMail /> <span>{resData[i].email}</span>
              </div>
              <div
                style={{
                  display: "flex",
              ...
              >
                <DollarOutlined />{" "}
                <span>
                  <strong>{resData[i].total}đ</strong>
                </span>
              </div>
              <div
                style={{
              ...
              >
                <FiClock /> <span>{resData[i].created}</span>
              </div>
              <div
                style={{
                  display: "flex",
              ...
              >
                <FaRegAddressBook />{" "}
                <span>{resData[i]?.address || "Chưa có dữ liệu"}</span>
              </div>
            </>
          ),
    ...
    },
    {
      ...
      render: (text, record) => (
        <Space size="middle">
          ...
          <a onClick={() => handleChangeStatus(record, "Not processed")}>
            <FcProcess style={{ color: "green", fontSize: "18px" }} />
          </a>
          <a onClick={() => handleChangeStatus(record, "Processing")}>
            <BsCheckCircle style={{ color: "green", fontSize: "18px" }} />
          </a>

          <a onClick={() => handleChangeStatus(record, "Shipped")}>
  ...
  const changeStatusInBoard = async (board, card, source, destination) => {
    const res = await orderApi.changeStatusOrderById(
      card.id,
